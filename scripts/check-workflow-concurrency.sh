...
# the caller also produces for itself, both land in one group, and when either
...
# called workflow's jobs missing, and no log or annotation names a cause.
#
# A literal prefix is the part a caller cannot reproduce by accident, because it
# names a file rather than a run.
#
...
for wf in .github/workflows/*.yml .github/workflows/*.yaml; do
  [ -f "$wf" ] || continue
  # Only `group:` lines inside a concurrency block. awk tracks the block so a
  # `group:` key belonging to anything else is not matched.
...
Keep the context parts that separate callers and refs; lead with the literal.
EOF
fi
